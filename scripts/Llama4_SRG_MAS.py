...
os.makedirs("MAS_logs", exist_ok=True)
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[
        logging.FileHandler(f"MAS_logs/agent_logs_Llama4_{ts}.log"),
...
def get_valid_srg(agent, question_id=None, is_agent1=False):
    srg_file = f"srg_cache/Llama4_srg_{question_id}.json" if question_id else None
    revmap_file = f"srg_cache/Llama4_reverse_map_{question_id}.json" if question_id and is_agent1 else None
...
def run_pipeline(question_id, question_text, rubric_text, encoded_student_image, image_size,golden_standard_images):
    ...
    if not analysis['terminate'] and analysis['priority_fix']:
        ...
        logger.info(f"Calling Llama4 to generate visual hint overlay {visual_hint}")
        ...
        try:
            ...
            output_path = "srg_cache/visual_hint_dynamic_Llama4.png"
