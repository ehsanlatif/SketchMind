...
from Llama4_SRG_Agents import Agent1, Agent2, Agent3, Agent4, SRGBuilder
...
def strip_json_wrapping(text: str) -> str:
    ...
    # Llama4 is not consistent in its json wrapping, so we need to handle both cases
    pattern = r"```(?:json)?\s*({.*?})\s*```"
...
def get_valid_srg(agent, question_id=None, is_agent1=False, cache_dir="srg_cache"):
    srg_file = os.path.join(cache_dir, f"Llama4_srg_{question_id}.json") if question_id else None
    revmap_file = os.path.join(cache_dir, f"Llama4_reverse_map_{question_id}.json") if question_id and is_agent1 else None
    ...
    while attempt < max_attempts:
        ...
        feedback = "Please correct the SRG, ensure the graph is connected and Bloom level, Ensure the Bloom levels are in the correct bottom up order."
        attempt += 1
...
def run_pipeline(question_id, question_text, rubric_text, encoded_student_image, image_size, golden_standard_images, config=None):
    """
    Run the Llama4-based multi-agent sketch evaluation pipeline.
    ...
    logger.info("Running Llama4 pipeline...")
    ...
    agent4 = Agent4(
        agent1_output=ref_srg.build_graph(), 
        reverse_mapping=agent1.reverse_mapping,
        golden_standard_images=golden_standard_images
    )
    feedback = agent4.run(analysis)
    logger.info("Feedback: %s", feedback)

    # Select ALL missing concepts to visualize (or top 3 to avoid clutter)
    if not analysis['terminate'] and analysis['priority_fix']:
        missing_concepts = analysis['priority_fix'][:2]  # Top 2 to avoid overwhelming
        concept_names = [c[0] for c in missing_concepts]
        logger.info(f"Calling Llama4 to generate visual hints for concepts: {concept_names}")
        updated_image_response =  agent4.generate_visual_overlay(encoded_student_image, missing_concepts, image_size, feedback)
        logger.info(f"Overlay image with Llama4 hint generated")
        ...
        try:
            ...
            exec(dedent(response_code), exec_globals, local_vars)
            exec(dedent(response_code), globals(), local_vars)

            overlay_hint = local_vars.get("overlay_hint")
            if not overlay_hint:
                raise ValueError("overlay_hint function not found in Llama4 response.")

            image = decode_image(encoded_student_image)
            modified_image = overlay_hint(image)
            os.makedirs(results_dir, exist_ok=True)
            output_path = os.path.join(results_dir, "visual_hint_dynamic_Llama4.png")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Running Llama4_SRG_MAS in standalone mode")
    print("For better modularity, use: python run.py --config config/task_config.yaml --model-type llama4")
    print("=" * 60)
    ...
    if os.path.exists(config_path):
        ...
        config = None
        student_image_path = "1_40185.jpg"
        ...
        golden_standard_images = [encode_image_to_base64(image_path) for image_path in golden_standard_images_path]
